...
git clone -b core https://github.com/vernesong/OpenClash.git
find OpenClash/dev/premium -name "clash-linux-amd64-20*"  | xargs -i mv -f {} ./
rm -rf OpenClash
gzip -df clash-linux-amd64-20*.gz && rm -rf clash-linux-amd64-20*.gz
